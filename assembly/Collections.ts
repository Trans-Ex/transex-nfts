...
export class Collections {
  ...
  mint(args: collections.mint_arguments): collections.empty_object {
    ...
    // assign the new token owner
    const newToken = new collections.token_object();
    let tokenId: string;

    for (let index = 0; index < <i32>args.number_tokens_to_mint; index++) {

      let hash = 0;
      for (let i = 0; i < to.length; i++) {
        let char = to[i];
        hash = ((hash << 5) - hash) + <i32>char;
        hash = hash & hash;
      }
      hash = ((hash << 5) - hash) + <i32>args.number_tokens_to_mint;
      hash = hash & hash;
      hash = ((hash << 5) - hash) + <i32>supply.value;
      hash = hash & hash;

      const indexCandidate = (hash % (Constants.MAX_SUPPLY - (index + supply.value)) + 1);

      let tokenCandidateCount = 0;
      
      let tokenCreatedFlag = false;

      for (let tokenIndex = 1; tokenIndex <= <i32>Constants.MAX_SUPPLY; tokenIndex++) {
        const token = this._state.getToken(tokenIndex.toString());
        if (!token) {
          tokenCandidateCount++;
        }
        if (tokenCandidateCount === indexCandidate) {
          ...
          tokenCreatedFlag = true;
          break;
        }

      }
      System.require(tokenCreatedFlag, "Unable to mint new token");
